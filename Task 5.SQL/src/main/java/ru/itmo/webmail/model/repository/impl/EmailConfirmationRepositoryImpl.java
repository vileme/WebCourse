...
public class EmailConfirmationRepositoryImpl implements EmailConfirmationRepository {
    ...
    public void emailToConfirm(User user) {
        DatabaseUtils.MakeRequestToDB("INSERT INTO EmailConfirmation (userId, secret, creationTime) VALUES (?, ?, NOW())",
                "Can't execute emailToConfirm", Long.toString(user.getId()),UUID.randomUUID().toString());
    }
    ...
    public boolean confirmBySecret(String secret) {
        ...
        DatabaseUtils.MakeRequestToDB("UPDATE User SET confirmed=TRUE WHERE id=?",
                "Can't find user to verify", userIdString);
        DatabaseUtils.MakeRequestToDB("DELETE FROM EmailConfirmation WHERE userId=?",
